describe("Juego", function() {
    ...
    describe("cuando un juego es creado", function() {
      ...
      it("deberian crearse los dos jugadores con sus respectivos valores", function() {
        ...
        expect(juego.jugadores[0].lado).toEqual("Norte");
        ...
        expect(juego.jugadores[1].lado).toEqual("Sur");
      ...
      it("deberia crearse el tablero con las respectivas fichas para cada jugador", function() {
        const tamanoTablero = juego.tablero.length;
        expect(tamanoTablero).toEqual(6);
        for (let i = 0; i < tamanoTablero; i++) for (let j = 0; j < tamanoTablero; j++) {
    			if((i%2 == 0 && j%2 != 0) || (i%2 != 0 && j%2 == 0)) {
    				if (i < tamanoTablero/2 - 1 || i > tamanoTablero/2) {
              expect(juego.getFicha(i, j) instanceof Peon).toBeTruthy();
	    				if (i < tamanoTablero/2 - 1) expect(juego.getFicha(i, j).jugador).toEqual(juego.jugadores[0]);
	    				if (i > tamanoTablero/2) expect(juego.getFicha(i, j).jugador).toEqual(juego.jugadores[1]);
    				} else {
